    ...
    public class BuildPipeline
    {
        ...
        public async Task<List<TestFailure>> GetTestFailures(Deployment deployment, CancellationToken cancel)
        {
            ...
            try
            {
                ...
                var buildArtifact = await buildClient.GetArtifactAsync(Ado.Project, this.buildId, BuildPipeline.ArtifactName, cancellationToken: cancel);
                ...
                var zipFilePath = Path.Combine(downloadFolder, $"{BuildPipeline.ArtifactName}_{this.buildId}.zip");
                ...
                using (var archive = ZipFile.OpenRead(zipFilePath))
                {
                    ...
                    if (entry != null)
                    {
                        ...
                        var failedTestCases = testCases.Where(t => !t.Succeeded)
                        ...
                        if (downloadTestFailures.Any() && !string.IsNullOrEmpty(deployment.LogShare))
                        {
                            DownloadTestFailure.InferTestFailureReason(deployment.LogShare, downloadTestFailures);
                        }

                        testFailures.AddRange(failedTestCases);
                    }
